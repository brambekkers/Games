...
(function(dice) {
    ...
    this.create_d4_materials = function(size, margin) {
        function create_d4_text(text, color, back_color) {
            ...
            canvas.width = canvas.height = ts;
            context.font = (ts - margin) / 1.5 + "pt Great Vibes";
            context.fillStyle = back_color;
            context.fillRect(0, 0, canvas.width, canvas.height);
            context.textAlign = "center";
            context.textBaseline = "middle";
    ...
    this.dice_color = '#540000';
    this.ambient_light_color = 0xf0f5fb;
    this.spot_light_color = 0xefdfd5;
    this.selector_back_colors = { color: 0x00000, shininess: 0, emissive: 0x858787 };
    this.desk_color = '#c3e6cb'; 
    ...
    this.create_d6 = function() {
        if (!this.d6_geometry) this.d6_geometry = this.create_d6_geometry(this.scale * 0.5);
    ...
    this.stringify_notation = function(nn) {
        var dict = {}, notation = '';
        for (var i in nn.set) 
            if (!dict[nn.set[i]]) dict[nn.set[i]] = 2;
        for (var i in dict) {
    ...
    this.dice_box = function(container, dimentions) {
        ...
        this.renderer.setClearColor("#c3e6cb", 1);
